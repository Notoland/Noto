    ...
    fn declare_class(&mut self, item: &Item, decl: &TypeDeclItem) {
        ...
        if decl.class_kind != ClassKind::Class {
            self.sink.emit(
                ...
                .with_primary(item.span, "not implemented in Noto 0.8")
        ...
        for param in &decl.primary_params {
            if let Some(default) = &param.default {
                self.sink.emit(
                    ...
                    .with_primary(default.span, "not implemented in Noto 0.8"),
                );
            }
    ...
    fn collect_method(&mut self, class: ClassId, item: &Item, function: &FnItem) {
        ...
        if !function.type_params.is_empty() {
            self.sink.emit(
                ...
                .with_primary(function.type_params[0].span, "not implemented in Noto 0.8"),
            );
            return;
        }
    ...
    fn collect_fn(&mut self, item: &Item, function: &FnItem) {
        if let Some(receiver) = &function.receiver {
            self.sink.emit(
                ...
                .with_primary(receiver.span, "not implemented in Noto 0.8"),
            );
            return;
        }
        if !function.type_params.is_empty() {
            ...
            TypeExprKind::Named { path, arguments } => {
                if !arguments.is_empty() {
                    self.sink.emit(
                        ...
                        .with_primary(ty.span, "not implemented in Noto 0.8"),
                    );
